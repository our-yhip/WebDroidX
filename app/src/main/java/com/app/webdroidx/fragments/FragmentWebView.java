package com.app.webdroidx.fragments;
...
import com.app.webdroidx.activities.MainActivity;
import com.app.webdroidx.databases.prefs.SharedPref;
import com.app.webdroidx.listener.DrawerStateListener;
import com.app.webdroidx.listener.LoadUrlListener;
import com.app.webdroidx.listener.WebViewOnTouchListener;
import com.app.webdroidx.utils.MyFragment;
import com.app.webdroidx.utils.Tools;
import com.app.webdroidx.webview.AdvancedWebView;
import com.app.webdroidx.webview.VideoEnabledWebChromeClient;
import com.app.webdroidx.webview.VideoEnabledWebView;
...
public class FragmentWebView extends MyFragment implements AdvancedWebView.Listener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            pageContent = getArguments().getString("content");
        }
        ...
        if (activity != null) {
            if (Tools.isOnline(activity)) {
                if (pageType.equals("content")) {
                    Tools.loadHtml(activity, webView, pageContent, true);
                } else {
                    if (pageUrl.contains("raw.githubusercontent.com")) {
                        new ParsePageTask().execute();
    ...
    public class ParsePageTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            Tools.loadHtml(activity, webView, html, false);
        }
    }
    ...
    public void refreshData() {
        if (activity != null) {
            if (Tools.isOnline(activity)) {
                ...
                if (!pageUrl.contains("about:blank")) {
                    webView.loadUrl(pageUrl);
                } else {
                    showProgress(false);
                    showSwipeRefresh(false);
            ...
            if (Tools.isDownloadableFile(url) || url.contains("?target=download")) {
                ...
                if (url.contains("target=video") || url.contains("target=audio") || url.contains("target=image")) {
                    ...
                    if (!external && !internal) {
                        external = Config.FORCE_TO_OPEN_LINK_IN_EXTERNAL_BROWSER;
                    }
                    if (external) {
